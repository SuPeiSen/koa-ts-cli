...
export class ProjectService {
    ...
    private static setupPrisma(projectPath: string, npmType: string) {
        ...
        execSync(`${npmType} install prisma @prisma/client`, { cwd: projectPath, stdio: "ignore" });

        Logger.info("Initializing prisma...");
        execSync("npx prisma init", { cwd: projectPath, stdio: "ignore" });
    }

    private static setupTypeORM(projectPath: string, npmType: string, templatePath: string) {
        Logger.info("Installing typeorm...");
        execSync(`${npmType} install typeorm reflect-metadata`, { cwd: projectPath, stdio: "ignore" });

        // Copy template files
        try {
            execSync(`cp ${path.join(templatePath, "data-source.txt")} ${path.join(projectPath, "src/data-source.ts")}`);
            execSync(`mkdir -p ${path.join(projectPath, "src/entity")} ${path.join(projectPath, "src/model")}`);
            execSync(`cp ${path.join(templatePath, "test.txt")} ${path.join(projectPath, "src/entity/test.ts")}`);
            execSync(`cp ${path.join(templatePath, "base_model.txt")} ${path.join(projectPath, "src/model/base_model.ts")}`);
        } catch (e) {
    ...
    private static updateGitignore(projectPath: string) {
        ...
        try {
            appendFileSync(gitignorePath, IGNORE_FILES.map((file) => `\n${file}`).join(""));
        } catch (e) {
            Logger.warn("Failed to update .gitignore");
        }
    }

    private static setupDocker(projectPath: string, useDocker: boolean, projectName: string) {
        if (!useDocker) {
