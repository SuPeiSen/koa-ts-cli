...
export class TemplateService {
    ...
    static async downloadTemplate(projectName: string, options: DownloadOptions) {
        ...
        const { npm_type = "npm", registry = "https://registry.npmjs.org/", x_get = false } = options;
    ...
    private static initializeGit(projectPath: string) {
        ...
        try {
            execSync("git init --initial-branch=main", { cwd: projectPath, stdio: "ignore" });
     ...
     * 安装项目依赖。
     */
    private static installDependencies(projectPath: string, npmType: string, registry: string) {
