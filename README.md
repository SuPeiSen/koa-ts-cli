...
生成的代码会自动包含 `koa-ts-core` 的 `@Router` 和 `@Validate` 装饰器模板。
...
2. 解析 `koa-ts-core` 的 `Symbol(route_metadata)` 元数据。
...
A: `doc` 命令仅生成文档**配置代码** (在 `doc/` 目录下)。你需要启动项目服务，通过 `koa-ts-core` 内置的 Swagger UI 中间件来访问文档页面 (默认路径 `/swagger-ui`)。

---

## 链接

- [koa-ts-core](https://www.npmjs.com/package/koa-ts-core): 核心框架库
- [Issues](https://github.com/SuPeiSen/koa-ts-cli/issues): 问题反馈
